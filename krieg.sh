...
Sync () {
  ...
  for i in "scripts" "OP5-OP5T" "AnyKernelBase" "Toolchains/aarch64-linux-android-4.9" "Toolchains/linux-x86"; do
    if [ -d "$i" ]; then
      ...
      echo ""
      case "$i" in
        "scripts") git clone --depth=1 https://github.com/Krieg-Kernel/scripts.git "$i";;
        "OP5-OP5T") git clone --depth=1 https://github.com/Krieg-Kernel/OP5-OP5T.git "$i";;
        "AnyKernelBase") git clone --depth=1 https://github.com/Krieg-Kernel/AnyKernelBase.git "$i";;
